...
class DailySummaryJobTest {
    ...
    void setup() {
        ...
        props = new AppProps(
                new AppProps.Mail("user", "pass",
                        ...
                        new AppProps.Mail.Smtp("smtp.naver.com", 465, "from@naver.com", List.of("to@naver.com"))),
                new AppProps.Gcal("", "", "Asia/Seoul", " - Preply lesson", 90),
                new AppProps.Supabase("url", "key", "preply_mail", true),
                new AppProps.Run(""),
                false);
        job = new DailySummaryJob(props, rateLoader, mailIngestionService, supabase, gcal, mailSender, fx);
    ...
    void supabase_enabled_uses_cache_only() {
        ...
        verify(mailIngestionService, never()).ingestYear();
        verify(mailIngestionService, never()).ingest(anyInt());
    ...
    void supabase_disabled_falls_back_to_imap_loaders() {
        ...
        when(rateLoader.loadTodayCancellationCompensations()).thenReturn(
                List.of(new RateEntry("bob", new Money(new BigDecimal("5"), "USD"), ZonedDateTime.now())));
